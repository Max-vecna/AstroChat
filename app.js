...
function handleMessageInputTyping() {
  ...
  const activeRoom = getActiveRoom();
  if (!activeRoom || isAiRoom(activeRoom) || !firebaseReady || !currentFirebaseUid) return;
...
async function publishTypingStatus(room, text) {
  if (!room?.id || isAiRoom(room) || !currentFirebaseUid || !firebaseReady) return;
...
async function clearCurrentTypingStatus() {
  ...
  const activeRoom = getActiveRoom();
  if (!activeRoom?.id || isAiRoom(activeRoom) || !currentFirebaseUid || !firebaseReady) return;
...
function openProfileSettingsModal() {
  ...
  requestAnimationFrame(() => profileNickInput.focus());
}
...
function updateUserHeader(statusText = null) {
  ...
  userSubtitle.textContent = statusText || (firebaseReady ? "Online" : "Conectando...");
}
    ...
    .forEach((friend) => {
      ...
      item.innerHTML = `
        ...
        <div class="friend-actions icon-only-actions">
          ...
          <button class="danger-action remove-friend-action icon-only-action" type="button" title="Remover amigo da lista" aria-label="Remover amigo da lista">
            <i class="fa-solid fa-user-minus" aria-hidden="true"></i>
          </button>
        </div>
      `;
...
function removeFriend(friend) {
  ...
  const shouldRemove = window.confirm(`Remover ${friend.nick} da sua lista de amigos?\n\nAs conversas existentes não serão apagadas para a outra pessoa.`);
  if (!shouldRemove) return;

  friends = friends.filter((item) => item.uid !== friend.uid);
  saveFriends();
  renderPrivateFriendsList();
  renderFriendsList();
  renderAll();
  showToast("Amigo removido", `${friend.nick} saiu da sua lista local de amigos.`);
}
...
async function searchUserForPrivateConversation() {
...
async function searchFirebaseUserAndRender({ nick, resultContainer, actionLabel, emptyText, onSelect }) {
  if (!firebaseReady || !currentFirebaseUid) {
...
function renderUserSearchResult(container, user, actionLabel, onSelect) {
  ...
  card.innerHTML = `
    <div class="avatar">${escapeHtml(user.avatar || getInitials(user.nick))}</div>
    <div class="friend-info">
